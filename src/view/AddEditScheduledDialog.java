...
public class AddEditScheduledDialog extends JDialog {
    ...
    private JPanel createFormPanel() {
        ...
        // ✅ Pre‑fill amount in display currency
        String amountText = "";
        if (existing != null) {
            double displayAmount = ExchangeRate.convert(existing.getAmount(), currencyCode);
            amountText = String.valueOf(displayAmount);
        }

        gbc.gridx = 0;
        addFormLabel(panel, gbc, "Amount (" + currencySymbol + ")");
        amountField = createStyledTextField(amountText);
        gbc.gridx = 1;
