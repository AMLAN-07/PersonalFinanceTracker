...
import utils.ExchangeRate;               // ✅ added
import javax.swing.*;
...
public class AddEditDebtDialog extends JDialog {
    ...
    public AddEditDebtDialog(JFrame parent, User user, Debt existing) {
        ...
        if (existing != null && existing.getEmiAmount() > 0) {
            ...
            // ✅ Show EMI in display currency
            double displayEMI = ExchangeRate.convert(existing.getEmiAmount(),
                    CurrencyFormatter.getCurrencyCode());
            emiField.setText(String.valueOf(displayEMI));
    ...
    private JPanel createFormPanel() {
        ...
        // ✅ Pre‑fill amounts in display currency
        String totalText = "";
        String remainingText = "";
        if (existing != null) {
            totalText = String.valueOf(ExchangeRate.convert(existing.getAmount(), currencyCode));
            remainingText = String.valueOf(ExchangeRate.convert(existing.getRemaining(), currencyCode));
        }

        gbc.gridx = 0;
        addFormLabel(panel, gbc, "Total Amount (" + currencySymbol + ")");
        amountField = createStyledTextField(totalText);
        gbc.gridx = 1;
        ...
        remainingField = createStyledTextField(remainingText);
        gbc.gridx = 1;
    ...
    private void saveDebt() {
        ...
        double remainingInINR = ExchangeRate.convertToINR(remaining, currencyCode);
        double emiInINR = ExchangeRate.convertToINR(emiAmount, currencyCode);
        ...
        d.setAmount(amountInINR);
        d.setRemaining(remainingInINR);
        d.setInterestRate(interest);
        d.setDueDate(due);
        d.setEmiAmount(emiInINR);
