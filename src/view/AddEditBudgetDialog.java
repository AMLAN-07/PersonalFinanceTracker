...
public class AddEditBudgetDialog extends JDialog {
    ...
    public AddEditBudgetDialog(JFrame parent, User user, Budget existing) {
        ...
        // ✅ Pre‑fill: show existing amount in the display currency, not INR
        String existingText = "";
        if (existing != null) {
            double displayAmount = ExchangeRate.convert(existing.getAmount(),
                    CurrencyFormatter.getCurrencyCode());
            existingText = String.valueOf((int) displayAmount);
        }
        amountField = new JTextField(existingText);
    ...
    private void saveBudget() {
        ...
        b.setAmount(amountInINR);                             // ✅ store INR
