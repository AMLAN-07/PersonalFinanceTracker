...
public class DebtsPanel extends JPanel {
    ...
    private void payDebt(Debt debt) {
        // Show default amount in display currency
        double defaultDisplayAmount = debt.getEmiAmount() > 0
                ? ExchangeRate.convert(debt.getEmiAmount(), CurrencyFormatter.getCurrencyCode())
                : ExchangeRate.convert(debt.getRemaining(), CurrencyFormatter.getCurrencyCode());
        String input = JOptionPane.showInputDialog(this,
                "Enter payment amount for '" + debt.getName() + "':",
                String.format("%.2f", defaultDisplayAmount));
        if (input == null || input.trim().isEmpty()) return;

        double displayAmount;
        try {
            displayAmount = Double.parseDouble(input.trim());
            if (displayAmount <= 0) {
        ...
        // Convert entered display‑currency amount to INR
        double amountInINR = ExchangeRate.convertToINR(displayAmount, CurrencyFormatter.getCurrencyCode());

        // Validate against remaining debt (stored in INR)
        if (amountInINR > debt.getRemaining()) {
            JOptionPane.showMessageDialog(this, "Cannot pay more than remaining balance.");
            return;
        }

        int confirm = JOptionPane.showConfirmDialog(this,
                String.format("Pay %s towards '%s'?", CurrencyFormatter.format(amountInINR), debt.getName()),
                "Confirm Payment", JOptionPane.YES_NO_OPTION);
        if (confirm != JOptionPane.YES_OPTION) return;

        // Create the expense transaction using INR amount
        ...
        // Reduce the debt remaining (stored in INR)
        debt.setRemaining(debt.getRemaining() - amountInINR);
