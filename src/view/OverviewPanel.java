...
public class OverviewPanel extends JPanel {
    ...
    private boolean budgetAlertShownThisSession = false;   // ✅ added
    ...
    public void refreshData() {
        ...
        // ✅ Budget alert notification (once per session)
        if (Settings.isBudgetAlertEnabled() && !budgetAlertShownThisSession) {
            List<String> overruns = new ArrayList<>();
            for (Budget b : budgets) {
                if (b.getPercentage() >= 100) {
                    overruns.add(b.getCategoryName() + " (" + b.getPercentage() + "%)");
                }
            }
            if (!overruns.isEmpty()) {
                String message = "Budget limit exceeded for:\n" + String.join("\n", overruns);
                JOptionPane.showMessageDialog(this, message, "Budget Alert", JOptionPane.WARNING_MESSAGE);
                budgetAlertShownThisSession = true;   // only show once
            }
        }
