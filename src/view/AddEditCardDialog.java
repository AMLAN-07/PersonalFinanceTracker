...
public class AddEditCardDialog extends JDialog {
    ...
    public AddEditCardDialog(JFrame parent, User user, Card card) {
        ...
        // ✅ Pre‑fill credit limit in display currency
        String limitText = "0";
        String balanceText = "0";
        if (existingCard != null) {
            limitText = String.valueOf(ExchangeRate.convert(existingCard.getCreditLimit(), currencyCode));
            balanceText = String.valueOf(ExchangeRate.convert(existingCard.getCurrentBalance(), currencyCode));
        }

        gbc.gridx = 0; gbc.weightx = 0;
        addFieldRow(formPanel, gbc, "Credit Limit (" + currencySymbol + "):");
        limitField = new JTextField(limitText);
        ...
        balanceField = new JTextField(balanceText);
    ...
    private void onSave() {
        ...
        // ✅ Convert entered display‑currency amounts to INR
        String currencyCode = CurrencyFormatter.getCurrencyCode();
        double limitInINR = ExchangeRate.convertToINR(limit, currencyCode);
        double balanceInINR = ExchangeRate.convertToINR(balance, currencyCode);
        ...
        card.setCreditLimit(limitInINR);
        card.setCurrentBalance(balanceInINR);
